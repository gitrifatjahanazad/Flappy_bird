    ...
    public partial class Form1 : Form
    {
        ...
        private void ReadAndShowScore()
        {
            using (StreamReader reader = new StreamReader("Score.ini"))
            {
                ...
                reader.Close();
                if (int.Parse(scoreLabel.Text) == 0 | int.Parse(scoreLabel.Text) > 0)
                {
                    scoreDifferent = score - int.Parse(scoreLabel.Text) + 1;
                }
                if (score < int.Parse(scoreLabel.Text))
                {
                    MessageBox.Show(string.Format("New High Score! \nPrevious Score: {0} \nNew Score: {1}", score, scoreLabel.Text), "Flappy Bird", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                if (score > int.Parse(scoreLabel.Text))
                {
                    MessageBox.Show(string.Format("You Lose! \nScore to win: {0} \nPrevious Score: {1}", scoreDifferent, score), "Flappy Bird", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }

                if (score == int.Parse(scoreLabel.Text))
                {
                    MessageBox.Show(string.Format("Your score {0}.", score), "Flappy Bird", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
